...
public class InterfaceStatsEditor extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		mCronExpressions = new CronExpressionEntry[] {
				...
				new CronExpressionEntry(getString(R.string.editor_cron_every_day), Configuration.CRON_EVERY_DAY),
	...
	protected Dialog onCreateDialog(int id) {
					...
					new DataPicker.OnBytesChangedListener() {
						...
						public void onChanged(DataPicker view, long oldValue, long newValue) {
							...
							values.put(InterfaceStatsColumns.BYTES_TRANSMISSION_LIMIT, Long
									.valueOf(newValue));
							values.put(InterfaceStatsColumns.NOTIFICATION_LEVEL, Integer.valueOf(0));
							// store values into the table
							getContentResolver().update(
									InterfaceStatsProvider.CONTENT_URI,
									values,
		...
		case DIALOG_RESET_COUNTERS:
			result = new AlertDialog.Builder(this).setMessage(R.string.editor_dialog_reset_counters).
				setPositiveButton(android.R.string.ok, new DialogInterface.OnClickListener() {

					@Override
					public void onClick(DialogInterface dialog, int which) {
						Resetter.broadcastResetIntent(getApplicationContext(), getIntent().getData());
					}
